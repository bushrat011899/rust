...
use crate::io::{
    self, BorrowedCursor, BufRead, IoHandle, IoSlice, IoSliceMut, Read, Seek, SeekFrom, Write,
};
...
impl<R: Read + ?Sized> Read for crate::sync::Arc<R>
where
    for<'a> &'a R: Read,
    R: IoHandle,
{
...
impl<A: Allocator> Read for VecDeque<u8, A> {
    ...
    fn read_to_string(&mut self, buf: &mut String) -> io::Result<usize> {
        ...
        unsafe { io::read::append_to_string(buf, |buf| self.read_to_end(buf)) }
    }
}
